...
bool init_from_markdown(Doc& d, const std::string& md_path) {
  bool ok = false;
  const std::string markdown = read_file_bytes(md_path, ok);
  if (!ok) {
    d.status = "cannot read " + md_path;
    return false;
  }
  reset_views(d);
  const fs::path work = make_workdir();
  ...
  // Copy relatively-referenced local images next to the source into assets/.
  const fs::path src_dir = fs::path(md_path).parent_path();
  std::size_t pos = 0;
  while ((pos = markdown.find("](", pos)) != std::string::npos) {
    const std::size_t start = pos + 2;
    const std::size_t end = markdown.find(')', start);
    if (end == std::string::npos) break;
    pos = end;
    const std::string ref = markdown.substr(start, end - start);
    if (ref.empty() || ref.find("://") != std::string::npos) continue;
    const fs::path rel(ref);
    if (rel.is_absolute()) continue;
    const fs::path from = src_dir / rel;
    if (!fs::is_regular_file(from, ec)) continue;
    const fs::path to = work / rel;  // preserve the reference as written
    fs::create_directories(to.parent_path(), ec);
    fs::copy_file(from, to, fs::copy_options::overwrite_existing, ec);
  }

  auto dir = mcdf::DirectoryContainer::open(work);
  if (!dir) {
    d.status = "import failed: " + dir.error().message;
    return false;
  }
  mcdf::Metadata meta;
  meta.created_at = iso8601_now_utc();
  if (auto r = mcdf::create_document(**dir, markdown, meta); !r) {
    d.status = "import failed: " + r.error().message;
    return false;
  }
  (void)mcdf::audit_append(**dir, "IMPORTED", "mcdf-studio",
                           iso8601_now_utc());
  ...
  d.is_archive = false;
  d.archive_path.clear();
  d.path.clear();  // untitled until the first Save
  load_from_workdir(d);
  d.status = "imported " + fs::path(md_path).filename().string() +
             " - Save to choose a file";
  return true;
}

bool init_from_epub(Doc& d, const std::string& epub_path,
                    mcdf::EpubImportReport* report_out) {
  bool ok = false;
  const std::string bytes = read_file_bytes(epub_path, ok);
  if (!ok) {
    d.status = "cannot read " + epub_path;
    return false;
  }
  reset_views(d);
  const fs::path work = make_workdir();
  ...
  if (!report) {
    d.status = "import failed: " + report.error().message;
    return false;
  }
  (void)mcdf::audit_append(**dir, "IMPORTED", "mcdf-studio", iso8601_now_utc());
  ...
  d.path.clear();  // untitled until the first Save
  load_from_workdir(d);
  d.status = "imported " + std::to_string(report->chapters) + " chapter(s), " +
             std::to_string(report->images) + " image(s) - Save to choose a file";
  return true;
}

bool init_from_html(Doc& d, const std::string& html_path,
                    mcdf::HtmlImportReport* report_out) {
  bool ok = false;
  const std::string html = read_file_bytes(html_path, ok);
  if (!ok) {
    d.status = "cannot read " + html_path;
    return false;
  }
  reset_views(d);
  const fs::path work = make_workdir();
  ...
  const fs::path source_dir = fs::path(html_path).parent_path();
  const auto resolver =
      [&source_dir](std::string_view rel) -> std::optional<std::string> {
    std::error_code ec;
    const fs::path base = fs::weakly_canonical(source_dir, ec);
    const fs::path real = fs::weakly_canonical(source_dir / fs::path(std::string(rel)), ec);
    if (ec) return std::nullopt;
    const auto rel_to_base = fs::relative(real, base, ec);
    if (ec || rel_to_base.empty() || *rel_to_base.begin() == "..")
      return std::nullopt;  // never read outside the page's directory
    bool read_ok = false;
    std::string bytes = read_file_bytes(real.string(), read_ok);
  ...
  d.path.clear();
  load_from_workdir(d);
  d.status = "imported HTML (" + std::to_string(report->images) +
             " image(s)) - Save to choose a file";
  return true;
}
