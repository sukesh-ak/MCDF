...
// here so it can be driven headlessly by tests and, later, from
...
// Import a Markdown file as a new untitled document (title from the first
// heading; sibling images referenced relatively are copied into assets/).
// Markdown is the native content format, so this is authoring, not
// conversion. False on read/create failure (d.status explains).
bool init_from_markdown(Doc& d, const std::string& md_path);

// Import an EPUB as a new untitled document (chapters in spine order, images
// into assets/, generated schema). `report_out` receives the human-readable
// conversion report. False on failure (d.status explains).
bool init_from_epub(Doc& d, const std::string& epub_path,
                    mcdf::EpubImportReport* report_out = nullptr);

// Import an HTML page as a new untitled document; relatively-referenced
// images are read from the page's own directory (never the network).
bool init_from_html(Doc& d, const std::string& html_path,
                    mcdf::HtmlImportReport* report_out = nullptr);
