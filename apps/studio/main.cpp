...
// offscreen crash check CI runs under xvfb.
bool g_smoke = false;
...
enum class OpenMode {
  ...
  ExportHtml, ExportText, ImportMarkdown, ImportEpub, ImportHtml,
};

// Conversion report from the last import, shown once as a modal so the user
// sees what a best-effort conversion could not carry over.
std::vector<std::string> g_import_notes;
std::string g_import_summary;
bool g_show_import_report = false;
...
// thread so the UI never blocks on crypto. One job owns a
...
// File > Import Markdown: wrap a .md file (and its local images) as a new
// untitled document.
void import_markdown(const std::string& md_path) {
  Document* d = new_document();
  studio::init_from_markdown(*d, md_path);
  sync_editor(*d);
}

// File > Import HTML: convert a page into a new untitled document.
void import_html(const std::string& html_path) {
  Document* d = new_document();
  mcdf::HtmlImportReport report;
  if (!studio::init_from_html(*d, html_path, &report)) {
    g_import_summary = d->status;
    g_import_notes.clear();
  } else {
    g_import_summary = (report.title.empty() ? std::string("Imported page")
                                             : "\"" + report.title + "\"") +
                       " - " + std::to_string(report.images) + " image(s)";
    g_import_notes = report.notes;
  }
  g_show_import_report = true;
  sync_editor(*d);
}

// File > Import EPUB: convert a book into a new untitled document, then show
// the conversion report.
void import_epub(const std::string& epub_path) {
  Document* d = new_document();
  mcdf::EpubImportReport report;
  if (!studio::init_from_epub(*d, epub_path, &report)) {
    g_import_summary = d->status;
    g_import_notes.clear();
    g_show_import_report = true;
    sync_editor(*d);
  ...
  sync_editor(*d);
  g_import_summary = "\"" + report.title + "\" - " +
                     std::to_string(report.chapters) + " chapter(s), " +
                     std::to_string(report.images) + " image(s)";
  g_import_notes = report.notes;
  g_show_import_report = true;
}
...
void open_import_markdown_dialog() {
  imfd::Config cfg;
  cfg.title = "Import Markdown";
  cfg.mode = imfd::Mode::OpenFile;
  cfg.filters = {{"Markdown", {".md", ".markdown", ".txt"}},
                 {"All files", {"*"}}};
  if (!g_last_dir.empty()) { cfg.start_dir = g_last_dir; g_last_dir.clear(); }
  g_pending = OpenMode::ImportMarkdown;
  g_dialog.Open(cfg);
}

void open_import_html_dialog() {
  imfd::Config cfg;
  cfg.title = "Import HTML";
  cfg.mode = imfd::Mode::OpenFile;
  cfg.filters = {{"HTML page", {".html", ".htm", ".xhtml"}}, {"All files", {"*"}}};
  if (!g_last_dir.empty()) { cfg.start_dir = g_last_dir; g_last_dir.clear(); }
  g_pending = OpenMode::ImportHtml;
  g_dialog.Open(cfg);
}

void open_import_epub_dialog() {
  imfd::Config cfg;
  cfg.title = "Import EPUB";
  cfg.mode = imfd::Mode::OpenFile;
  cfg.filters = {{"EPUB book", {".epub"}}, {"All files", {"*"}}};
  if (!g_last_dir.empty()) { cfg.start_dir = g_last_dir; g_last_dir.clear(); }
  g_pending = OpenMode::ImportEpub;
  g_dialog.Open(cfg);
}
...
// Imports are best-effort conversions - say plainly what did not come across.
void draw_import_report() {
  if (g_show_import_report) {
    ImGui::OpenPopup("Import report");
    g_show_import_report = false;
  }
  const ImGuiViewport* vp = ImGui::GetMainViewport();
  ImGui::SetNextWindowPos(vp->GetCenter(), ImGuiCond_Appearing, ImVec2(0.5f, 0.5f));
  ImGui::SetNextWindowSize(ImVec2(520.0f, 0.0f), ImGuiCond_Appearing);
  bool open = true;
  if (!ImGui::BeginPopupModal("Import report", &open,
                              ImGuiWindowFlags_AlwaysAutoResize))
    return;
  ImGui::TextWrapped("%s", g_import_summary.c_str());
  if (!g_import_notes.empty()) {
    ImGui::Spacing();
    ImGui::SeparatorText("Not carried over");
    for (const auto& note : g_import_notes) ImGui::BulletText("%s", note.c_str());
    ImGui::Spacing();
    ImGui::TextDisabled("Review the document, then Save to choose a file.");
  }
  ImGui::Spacing();
...
void draw_menu_bar() {
  ...
  if (ImGui::BeginMenu("File")) {
    ...
    if (ImGui::MenuItem(ICON_FA_FOLDER "  Open unpacked folder..."))
      open_folder_dialog();
    if (ImGui::MenuItem(ICON_FA_FILE_IMPORT "  Import Markdown..."))
      open_import_markdown_dialog();
    if (ImGui::MenuItem(ICON_FA_BOOK "  Import EPUB..."))
      open_import_epub_dialog();
    if (ImGui::MenuItem(ICON_FA_CODE "  Import HTML..."))
      open_import_html_dialog();
...
void draw_encryption_panel() {
  ...
  if (ImGui::Begin(ICON_FA_LOCK "  Encryption###Encryption", &g_show_encryption)) {
    ...
    if (!dp) {
      ...
      // The structure attestation (spec 5.2.1). Worth showing, because it is the
      // only reason a sealed document still validates against its schema: a
      // reader without the key checks the required sections against this list
      // instead of against headings it cannot see.
      if (p.structure.present) {
        ImGui::TextDisabled("Structure attested:");
        for (const auto& id : p.structure.bound_sections)
          ImGui::BulletText("%s", id.c_str());
        if (p.structure.bound_sections.empty())
          ImGui::BulletText("(no sections bound)");
        if (ImGui::IsItemHovered())
          ImGui::SetTooltip(
              "Sections that bound to a heading when this document was sealed.\n"
              "Validators evaluate the schema against this list; decrypting\n"
              "re-checks it against the real headings.");
      } else if (std::find(p.encrypted_files.begin(), p.encrypted_files.end(),
                           "content.md") != p.encrypted_files.end()) {
        // Sealed content with no attestation: a validator cannot evaluate the
        // schema at all, and says so as E_POLICY_INVALID against this document.
        ImGui::TextDisabled("Structure attested: none");
        if (ImGui::IsItemHovered())
          ImGui::SetTooltip(
              "content.md is sealed and nothing records which sections bound\n"
              "before it was. A reader cannot check the schema (spec 5.2.1).");
      }
...
int main(int argc, char** argv) {
  ...
  while (!glfwWindowShouldClose(window) && !g_quit) {
    ...
    if (dlg_res == imfd::Result::Picked) {
      ...
      else if (g_pending == OpenMode::ImportMarkdown) import_markdown(picked);
      else if (g_pending == OpenMode::ImportEpub) import_epub(picked);
      else if (g_pending == OpenMode::ImportHtml) import_html(picked);
