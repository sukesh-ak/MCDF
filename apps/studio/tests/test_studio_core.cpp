...
//
// Headless tests over Studio's document engine: the same
...
TEST_CASE("markdown import wraps a file and copies its local images") {
  const fs::path src = studio::make_workdir();
  REQUIRE(studio::write_file_bytes((src / "note.md").string(),
                                   "# Imported\r\n\r\n![pic](assets/p.png)\r\n"));
  std::error_code ec;
  fs::create_directories(src / "assets", ec);
  REQUIRE(studio::write_file_bytes((src / "assets" / "p.png").string(), "PNG"));

  studio::Doc d;
  REQUIRE(studio::init_from_markdown(d, (src / "note.md").string()));
  CHECK(d.has_content);
  CHECK(d.path.empty());              // untitled until first save
  CHECK(d.content == "# Imported\n\n![pic](assets/p.png)\n");  // canonical
  CHECK(d.title == "Imported");       // from the first heading
  CHECK(d.has_manifest);
  CHECK(d.manifest.files.contains("assets/p.png"));  // image came along
  studio::refresh_audit(d);
  REQUIRE(d.audit.size() == 1);
  CHECK(d.audit[0].action == "IMPORTED");
